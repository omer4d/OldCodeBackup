...
void fillRow(std::list<Point>& queue, int mat[BOARD_W][BOARD_H], int x0, int y0, int src, int dest)
{
   int x, firstX, lastX;
   bool lastTopFillable = false, lastBottomFillable = false;

   for(x = x0 - 1; x >= 0 && mat[x][y0] == src; --x)
      mat[x][y0] = dest;
   
   firstX = x + 1;
   
   for(x = x0; x < BOARD_W && mat[x][y0] == src; ++x)
      mat[x][y0] = dest;
