...
struct Vec2f {
    ...
    // Will point inside a CW polygon!
    ...
    Vec2f toBase(Vec2f const& t, Vec2f const& n)
    {
        return Vec2f(this->dot(t), this->dot(n));
    }
    
    Vec2f fromBase(Vec2f t, Vec2f n)
    {
        return t * x + n * y;
    }
    
    static float ang(Vec2f const& v1, Vec2f const& v2)
    {
        return atan2(v1.cross(v2), v1.dot(v2));
    }
    
    static Vec2f slerp(Vec2f const& v1, Vec2f const& v2, float k)
    {
        float a = ang(v1, v2);
        return v1.rotated(a * k);
    }
