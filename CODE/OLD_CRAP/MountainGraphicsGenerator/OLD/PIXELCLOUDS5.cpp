...
void processHeightMap(Buffer<float>& heightMap, Buffer<float>& tempf)
{
   ...
   guassBlur<float>(heightMap, tempf, 3.f);
...
{   
    ...
    for(int j = clip.miny; j < clip.maxy; j++)
      ...
      void draw(BITMAP* bmp, float r)
      {
            heightMap.fill(0);
            genClouds(heightMap, Vec2f(), r, randf(0.3f, 0.7f), 3);   
            processHeightMap(heightMap, tempf);
            cellShaded(bmp, heightMap, 10.f, Vec3f(5, 20, -10).unit());
      }
};
...
{   
    ...
    PixelCloud pixelCloud(128, 64);
    BITMAP* cloud = create_bitmap(128, 64);
