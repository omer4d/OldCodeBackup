...
int raytraceVisual(GridMatrix<int>& grid, Vec2f orig, Vec2f dir, int refs = 0)
{
     ...
     while(y >= 0 && y < grid.rowNum)
     {
        ...
        if(tMaxX < tMaxY)
        {
           ...
           {
              if(refs > 0)
                hit = raytraceVisual(grid, realOrig + realDir * (t * 0.999f) * cellW, Vec2f(-realDir.x, realDir.y), refs - 1);
...
HitInfo raytrace(GridMatrix<int>& grid, Vec2f orig, Vec2f dir, int refs = 0)
{
     float cellW = grid.cellSize * 0.5f;
     float cellH = grid.cellSize * (float)sqrt(3.f) * 0.5f;
     ...
     while(y >= 0 && y < grid.rowNum)
     {
        ...
        if(grid.at(i, y) != 0)
        {
            ...
            return HitInfo(i, y, lastI, lastJ);
        }
        ...
        if(tMaxX < tMaxY)
        {
           ...
           {
              if(refs > 0)
                return raytrace(grid, realOrig + realDir * (t * 0.999f) * cellW, Vec2f(-realDir.x, realDir.y), refs - 1);
           }
        }
...
int main()
{
    ...
    { 
     ...
     float range = 2.5f;
     float da = range / (RES + 1);
     Vec2f dir = Vec2f(0.f, -1.f).rotated(-range / 2);
     
     /*
     HitInfo hitInfos[RES];
     
     for(int i = 0; i < RES; ++i)
     {
        dir = dir.rotated(da);
        hitInfos[i] = raytrace(result, src, dir, 1);
        raytraceVisual(result, src, dir, 1);
     ...
     for(int i = 0; i < RES; ++i)
       if(hitInfos[i].hit)
       {
          HitInfo const& info = hitInfos[i];
         circle(buffer, int(mat.toWx(info.parkI, info.parkJ)),
                        int(mat.toWy(info.parkJ)), mat.cellSize / 2, makecol(255, 255, 255));
       }*/
       
       
     raytraceVisual(result, src, (Vec2f(mouse_x, mouse_y) - src).unit(), 1);
     HitInfo hitInfo = raytrace(result, src, (Vec2f(mouse_x, mouse_y) - src).unit(), 1);
     
     if(hitInfo.hit)
     {
        circlefill(buffer, int(mat.toWx(hitInfo.parkI, hitInfo.parkJ)),
                        int(mat.toWy(hitInfo.parkJ)), 5, makecol(255, 255, 255));
     
     circlefill(buffer, int(mat.toWx(hitInfo.hitI, hitInfo.hitJ)),
                        int(mat.toWy(hitInfo.hitJ)), 5, 0);
     }
